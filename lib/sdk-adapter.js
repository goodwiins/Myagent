...
2. Parse findings and categorize by priority (P1 critical → P4 docs)
3. Use linear__list_teams to get the team ID
4. Use linear__create_issue to create issues directly in Linear for each finding
5. Optionally delegate to auto-fixer agent for safe fixes
6. Report summary with issue IDs and fix status
...
IMPORTANT: Always use the Linear MCP tools (linear__create_issue, linear__list_teams) to create real issues.
Do NOT just write to text files - actually create Linear issues.`,
    tools: [
      'Bash',
      'Read',
      'Glob',
...
2. Read the affected file completely using Read tool
3. Understand the proposed fix and verify it makes sense
4. Apply the fix using Edit tool
5. Run verification: linters, type checks, tests
6. If verification fails, revert and mark for manual review
7. Use linear__update_issue to update issue status (Done if fixed, add comment if failed)
8. Use linear__create_comment to document what was done
