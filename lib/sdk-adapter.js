...
Your workflow:
1. Run CodeRabbit CLI to analyze code changes: \`coderabbit review --type uncommitted --plain\`
2. Use serena__get_symbols_overview to understand code structure
3. Parse findings and categorize by priority (P1 critical → P4 docs)
4. Use linear__list_teams to get the team ID
5. Use linear__create_issue to create issues directly in Linear for each finding
6. Use serena__write_memory to store findings in .serena/memories/coderabbit_findings.md
7. Optionally delegate to auto-fixer agent for safe fixes
8. Report summary with issue IDs and fix status
...
IMPORTANT:
- Use Linear MCP tools (linear__create_issue, linear__list_teams) to create real issues
- Use Serena MCP tools (serena__*) for code analysis and memory storage
- Do NOT just write to text files - use the MCP tools!`,
    tools: [
      'Bash',
      'Read',
      'Glob',
...
Your workflow:
1. Use linear__get_issue to read the issue details from Linear
2. Use serena__find_symbol to locate the affected code
3. Use serena__read_file to read the affected file with full context
4. Understand the proposed fix and verify it makes sense
5. Use serena__read_memory to check auto_fix_patterns.md for similar fixes
6. Apply the fix using serena__replace_symbol_body or serena__replace_content
7. Run verification: linters, type checks, tests
8. If verification fails, revert and mark for manual review
9. Use linear__update_issue to update issue status (Done if fixed, add comment if failed)
10. Use linear__create_comment to document what was done
11. Use serena__write_memory to update auto_fix_patterns.md with successful patterns
...
IMPORTANT:
- Use Serena MCP tools (serena__*) for code analysis and editing
- Use Linear MCP tools for issue updates
- Store successful fix patterns in Serena memory

Return: { fixed: [{ issueId, file, verified }], failed: [{ issueId, reason }], errors: [] }`,
    tools: [
      'Read',
      'Edit',
      'Bash',
