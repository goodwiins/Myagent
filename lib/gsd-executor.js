...
import { runSubagent, EXECUTION_MODE } from './subagent-runner.js';
...
export class GsdExecutor {
  constructor(options = {}) {
    ...
    // Execution mode for tasks (simulated for testing, agent for real execution)
    this.executionMode = options.executionMode || EXECUTION_MODE.SIMULATED;

    // Safe git service (no shell injection)
    this.gitService = options.gitService || new GitService({
      cwd: this.basePath,
      dryRun: this.dryRun,
    });

    // Optional executor function for agent mode (SDK integration)
  ...
  async executePlan(options = {}) {
    ...
    return this._executeTasks(parsed.tasks, {
      results,
      phase,
      plan,
      sessionId,
   ...
   * @returns {Promise<object>} Execution result
   */
  async resumeCheckpoint(options = {}) {
    const {
      planPath,
      checkpointId,
      approved,
    ...
    if (!parsed.valid) {
      throw new Error(parsed.error || 'Invalid plan');
    }
    ...
    // Find the checkpoint task and the remaining tasks
    const checkpointIndex = parsed.tasks.findIndex(t => t.id === checkpointId);
    if (checkpointIndex === -1) {
      throw new Error(`Checkpoint not found: ${checkpointId}`);
    }

    // Initialize results with previous tasks (if we could track them)
    // For now, we'll re-run or assume we're continuing from here
    const results = {
      success: true,
      planPath,
      strategy: executionStrategy,
    ...
    // Add the checkpoint task itself as completed
    results.tasks.push({
      id: checkpointId,
      name: parsed.tasks[checkpointIndex].whatBuilt || 'Checkpoint',
      status: approved ? TASK_STATUS.COMPLETED : TASK_STATUS.FAILED,
      result: checkpointData,
    });
    ...
    if (!approved) {
      return {
        ...results,
        success: false,
        error: 'Checkpoint not approved',
      };
    }

    // Resume from next task
    const remainingTasks = parsed.tasks.slice(checkpointIndex + 1);
    const startTime = Date.now();

    return this._executeTasks(remainingTasks, {
      results,
      phase,
      plan,
      sessionId,
  ...
  /**
   * Internal method to execute a list of tasks
   * @private
   */
  async _executeTasks(tasks, context) {
    const {
      results,
      phase,
      plan,
    ...
    for (const task of tasks) {
      ...
      if (task.type?.startsWith('checkpoint:')) {
        ...
        return {
          ...
          tasksRemaining: tasks.length - (tasks.indexOf(task) + 1),
        };
      }
    ...
    if (!dryRun && results.tasks.some(t => t.commitHash)) {
      const metadataCommit = await this._createMetadataCommit(phase, plan, results.planPath);
   ...
   *
   * Task execution flow:
   * 1. Parse files from task definition
   * 2. Execute task via subagent (real or simulated)
   * 3. Run verification command if specified
   * 4. Create atomic commit for the task
   */
  async _executeTask(task, context) {
    ...
    try {
      ...
      // Execute task via subagent (real implementation)
      const rawSubagentResult = await this._executeTaskViaSubagent(task, context);

      // Validate subagent result structure (GOO-142)
      const subagentResult = this._validateSubagentResult(rawSubagentResult);
      result.subagentResult = subagentResult;

      // Update files modified from subagent result
      if (subagentResult.filesModified.length > 0) {
        result.filesModified = [
          ...new Set([...result.filesModified, ...subagentResult.filesModified]),
        ];
      }

      // Check if subagent execution succeeded
      if (subagentResult.status === 'error') {
        result.deviations.push({
          rule: DEVIATION_RULES.BLOCKER,
          category: 'execution_failed',
          description: `Task execution failed: ${subagentResult.error}`,
          task: task.id,
          autoFixed: false,
        });
      }
      ...
      if (result.filesModified.length > 0 && !this.dryRun) {
   ...
   *
   * This method handles the actual task execution by delegating to
   * the subagent runner, which supports both simulated and real agent modes.
   *
   * GOO-144: Added comprehensive error handling to differentiate error types.
   *
   * @param {object} task - Task to execute
   * @param {object} context - Execution context
   * @returns {Promise<object>} Subagent result
   */
  async _executeTaskViaSubagent(task, context) {
    const { phase, plan, sessionId } = context;
    const TIMEOUT_MS = 5 * 60 * 1000; // 5 minute timeout per task

    // Build subtask for subagent runner
    const subtask = {
      id: task.id,
      planId: `${phase}-${plan}`,
      description: task.action || task.name,
    ...
    try {
      // Execute via subagent runner with timeout
      const result = await runSubagent(subtask, sessionId, {
      ...
      // GOO-144: Comprehensive error handling
      ...
      if (errorMessage.includes('timeout') || errorMessage.includes('ETIMEDOUT')) {
        ...
        recoverable = false; // Task took too long, likely stuck
      } else if (errorMessage.includes('ECONNREFUSED') || errorMessage.includes('ENOTFOUND') || errorMessage.includes('network')) {
        errorType = 'network_error';
        recoverable = true; // Retry might help
      } else if (errorMessage.includes('validation') || errorMessage.includes('invalid') || errorMessage.includes('schema')) {
        errorType = 'validation_error';
        recoverable = false; // Fix input before retry
      } else if (errorMessage.includes('ENOENT') || errorMessage.includes('not found')) {
        errorType = 'resource_error';
        recoverable = false; // Missing file/resource
      }
   ...
   *
   * Uses GitService for safe git operations (no shell injection).
   *
  ...
  async commitTask(options) {
    ...
    try {
      // Stage and commit using safe GitService (no shell injection)
      const commitResult = await this.gitService.stageAndCommit(
        existingFiles,
        message,
      );
   ...
   *
   * Uses spawn for safer command execution.
   * Note: This still executes shell commands but with proper timeout handling.
   */
  async _runVerification(verifyCommand) {
    const { spawn } = await import('node:child_process');

    return new Promise((resolve) => {
      // Parse command - simple split for common verification commands
      // For complex commands with pipes, we need shell
      const needsShell = verifyCommand.includes('|') ||
                         verifyCommand.includes('&&') ||
                         verifyCommand.includes('>');

      let child;
      if (needsShell) {
        // Use sh -c for complex commands
        child = spawn('sh', ['-c', verifyCommand], {
          cwd: this.basePath,
      ...
      // Explicit timeout with process cleanup (GOO-141)
      const timeoutTimer = setTimeout(() => {
        ...
        // Fallback to SIGKILL after 5 seconds if process doesn't exit
...
- **Phase**: ${phase}, Plan ${plan} (Task ${taskResult.id})
- **Type**: tech-debt
- **Effort**: M
- **Priority**: medium
...
`;
   ...
   *
   * Uses GitService for safe git operations.
   */
  async _createMetadataCommit(phase, plan, planPath) {
    try {
      ...
      const result = await this.gitService.stageAndCommit(
        [summaryPath],
        message,
      );

      return result.success ? result.commitHash : null;
    } catch {
      return null;
    }
  }
...
`,
  ...
  /**
   * Validate and normalize subagent result structure (GOO-142)
   *
   * Ensures the result has expected properties with safe defaults
   * to prevent runtime errors from malformed responses.
   *
   * @param {*} rawResult - Raw result from subagent
   * @returns {object} Normalized result with guaranteed structure
   */
  _validateSubagentResult(rawResult) {
    // Handle null/undefined/non-object results
    if (!rawResult || typeof rawResult !== 'object') {
      return {
        status: 'error',
        error: 'Invalid subagent result: expected object',
        filesModified: [],
        output: null,
      };
    }

    // Normalize and provide defaults for expected properties
    return {
      status: typeof rawResult.status === 'string' ? rawResult.status : 'unknown',
      error: rawResult.error?.message || rawResult.error || null,
      filesModified: Array.isArray(rawResult.filesModified) ? rawResult.filesModified : [],
      output: rawResult.output ?? null,
