...
export class PriorityQueue {
  ...
  markCompleted(result = {}) {
    ...
    this.currentItem._queueMeta.state = ITEM_STATE.COMPLETED;
    this.currentItem._queueMeta.completedAt = new Date().toISOString();
    this.currentItem._queueMeta.result = result;

    this.processed.push(this.currentItem);
    this.currentItem = null;
  ...
  markFailed(error) {
    ...
    this.currentItem._queueMeta.attempts++;
    this.currentItem._queueMeta.lastError = error instanceof Error ? error.message : error;

    if (this.currentItem._queueMeta.attempts < this.maxRetries) {
      // Re-queue for retry (at same priority position)
      this.currentItem._queueMeta.state = ITEM_STATE.PENDING;
      const insertIndex = this._findInsertIndex(this.currentItem._queueMeta.priority);
      this.items.splice(insertIndex, 0, this.currentItem);
    } else {
      // Max retries exceeded
      this.currentItem._queueMeta.state = ITEM_STATE.FAILED;
      this.currentItem._queueMeta.failedAt = new Date().toISOString();
  ...
  getStats() {
    ...
    return {
      ...
      total: this.items.length + this.processed.length + this.failed.length + this.skipped.length,
      byPriority: {
