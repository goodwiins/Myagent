...
export class SyncManager {
  ...
  constructor(options = {}) {
    ...
    this._ensureSyncDir();
    this.activity = new SyncActivity({ syncPath: this.syncPath });
  }
  ...
  export(options) {
    ...
    // Log activity
    this.activity.logExport(normalizedLlm, message || `Exported context (${role || 'full'})`, handoff.stats);
  ...
  import(options) {
    ...
    // Log activity (determine importing LLM from caller context or use 'unknown')
    const importingLlm = this._currentLlm || 'unknown';
    this.activity.logImport(importingLlm, handoff.exportedBy, handoff.stats);
  ...
  merge(options) {
    ...
    // Log activity
    const mergingLlm = this._currentLlm || 'unknown';
    const sourceNames = handoffs.map(h => h.importedFrom);
    this.activity.logMerge(mergingLlm, sourceNames, strategy);
  ...
  /**
   * Set the current LLM identifier for activity logging
   * @param {string} llm - LLM identifier
   */
  setCurrentLlm(llm) {
    this._currentLlm = llm?.toLowerCase() || null;
  }
   ...
   */
  getActivity(options = {}) {
    return this.activity.getActivity(options);
  }
   ...
   */
  getActivitySummary() {
    return this.activity.getSummaryByLLM();
  }

  /**
   * Get freshness status for an LLM
   * @param {string} llm - LLM identifier
   * @returns {Object} Freshness info
   */
  getActivityFreshness(llm) {
    return this.activity.getFreshness(llm);
  }
}
