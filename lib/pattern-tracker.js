...
export class PatternTracker {
  ...
  _recordHistory(patternId, success, context = {}) {
    ...
    try {
      const fd = existsSync(historyPath)
        ? readFileSync(historyPath, 'utf-8')
        : '';
      writeFileSync(historyPath, fd + line);
    } catch {
      // Ignore write errors
    }
  }
  ...
  _extractTemplate(before, after) {
    ...
    return `${after.slice(0, prefixLen)}\${change}${after.slice(after.length - suffixLen || undefined)}` || after;
  }
