...
export class PhaseManager {
  ...
  /**
   * Plan a phase with tasks
   * @param {object} options - Planning options
   * @param {number|string} options.phase - Phase number or name (defaults to next unplanned)
   * @param {object} options.sessionContext - Session context for additional info
   * @param {number} options.maxTasksPerPlan - Max tasks per plan (default: 3)
   * @param {boolean} options.includeCodebaseAnalysis - Whether to analyze codebase
   * @param {object[]} options.tasks - Pre-defined tasks (optional)
   * @param {object} options.objective - Plan objective
   * @returns {Promise<object>} Planning result
   */
  async planPhase(options = {}) {
    const {
      phase,
      maxTasksPerPlan = 3,
      tasks = [],
    ...
    // Get or determine target phase
    let targetPhase;
    if (phase) {
      targetPhase = await this.getPhase(phase);
    } else {
      targetPhase = await this.getCurrentPhase();
    }

    if (!targetPhase) {
      return { error: 'No phase found to plan' };
    }

    // If tasks provided, create plan directly
    if (tasks.length > 0) {
      const result = await this.createPlan({
        phase: targetPhase.number,
        tasks: tasks.slice(0, maxTasksPerPlan),
        objective,
      });
      return {
        ...result,
    ...
    // Return phase info for manual planning
    return {
      phase: targetPhase.number,
      phaseName: targetPhase.name,
      status: targetPhase.status,
      existingPlans: targetPhase.plans.length,
      message: 'Provide tasks array to create a plan',
    };
  }

  /**
   * Create a multi-task plan for a phase
   * @param {object} options - Plan options
   * @param {number|string} options.phase - Phase number or name
   * @param {object} options.objective - Plan objective with description, purpose, output
   * @param {object[]} options.tasks - Array of task definitions
   * @param {string[]} options.contextFiles - Additional context files
   * @param {string[]} options.verification - Verification checklist
   * @param {string[]} options.successCriteria - Success criteria items
   * @returns {Promise<object>} Created plan info
   */
  async createMultiTaskPlan(options = {}) {
    const {
      phase,
      objective,
      tasks = [],
    ...
    const phaseInfo = await this.getPhase(phase);
    if (!phaseInfo) {
      return { error: `Phase not found: ${phase}` };
    }

    // Create plan with enhanced content
    const result = await this.createPlan({
      phase: phaseInfo.number,
      tasks,
      objective,
  ...
  /**
   * Update ROADMAP.md with current phase progress
   * @param {object} options - Update options
   * @param {string} options.milestone - Milestone name to update
   * @param {string} options.targetDate - Target date to update
   * @returns {Promise<object>} Update result
   */
  async updateRoadmap(options = {}) {
    const { milestone, targetDate } = options;
    const phases = await this.listPhases();

    // Calculate progress
    const totalPhases = phases.length;
    const completedPhases = phases.filter(p => p.status === PHASE_STATUS.COMPLETE).length;
    ...
    return {
      ...
      phases: {
        ...
        inProgress: phases.filter(p => p.status === PHASE_STATUS.IN_PROGRESS).length,
        pending: phases.filter(p => p.status === PHASE_STATUS.PENDING).length,
      },
      progress,
      message: 'Roadmap synced with phase status',
    };
  }
