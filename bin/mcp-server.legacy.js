...
try {
  ...
  contextStore = {
    query: () => [],
    addFinding: () => ({ added: false }),
    updateFinding: () => false,
    findSimilar: () => [],
    exportToMarkdown: () => '# No findings\n\nContext store not initialized.',
    getStats: () => ({ findings: 0, initialized: false }),
  };
}
...
try {
  ...
  patternTracker = {
    recommend: () => [],
    recordSuccess: () => ({ recorded: false }),
    recordFailure: () => ({ recorded: false }),
    addPattern: () => ({ added: false }),
    getStats: () => ({ patterns: 0, initialized: false }),
  };
}
...
try {
  ...
  contextFileManager = {
    read: () => ({ success: false, error: 'ContextFileManager not initialized' }),
    write: () => ({ success: false, error: 'ContextFileManager not initialized' }),
    status: () => ({ initialized: false }),
    init: () => ({ success: false, error: 'ContextFileManager not initialized' }),
    updateState: () => ({ success: false, error: 'ContextFileManager not initialized' }),
    addSummary: () => ({ success: false, error: 'ContextFileManager not initialized' }),
    getAutoLoadContext: () => ({ content: '', files: [], tokenCount: 0 }),
  };
}
...
try {
  ...
  planExecutor = {
    sessionManager: null,
    createPlan: () => ({ id: null, status: 'failed', error: 'PlanExecutor not initialized' }),
    execute: () => ({ success: false, error: 'PlanExecutor not initialized' }),
    getStatus: () => ({ status: 'unavailable', error: 'PlanExecutor not initialized' }),
    getSubtaskResult: () => null,
    cancel: () => ({ success: false, error: 'PlanExecutor not initialized' }),
  };
}
...
`;
      ...
      case 'goodflows_preflight_check': {
        try {
          const { action, findings, sessionId, team: _team, linearIssues, options = {} } = args;
          const {
